...
response.raise_for_status()domains = response.text.strip().split('\n')
...
final_domain = extract_full_domain(final_url, site_key)return domain, final_domain
except requests.RequestException as e:
...
update_json_file()
d) Tasto verde sulla destra Commit changes, e confermare commit changes
nella schermata successiva
e) Di nuovo Add file e Create new file
f) Come nome copiare e incollare .github/workflows/updatedomain.yml
g) Nello spazio sotto copiare e incollare il seguente testo:
name: Update Domains
on:
schedule:
...
jobs:
update-domains:runs-on: ubuntu-latest
steps:
...
git fetch --all
BRANCHES=$(git branch -r | grep -v '\->' | sed 's/origin\///')for branch in $BRANCHES
do
...
git pull --rebase origin $branch || (echo "Rebase fallito, tentando un merge" && git pull
origin $branch --no-rebase)
